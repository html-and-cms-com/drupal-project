...
if [ ! -f $DOCUMENTROOT/sites/default/settings.php ]
  then
    ...
    echo "\$config_directories[CONFIG_SYNC_DIRECTORY] = '../config/sync';" >> $DOCUMENTROOT/sites/default/settings.php
    echo "\$settings['hash_salt'] = '$(php -r 'print bin2hex(openssl_random_pseudo_bytes(32, $cstrong = TRUE));')';" >> $DOCUMENTROOT/sites/default/settings.php
fi
